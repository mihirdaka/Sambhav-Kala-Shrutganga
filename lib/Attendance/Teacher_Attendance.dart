...
class tAttendance_Home extends StatefulWidget {

 
  tAttendance_Home({Key key, this.tid}) : super(key: key);
  ...
  @override
  _tAttendance_HomeState createState() => _tAttendance_HomeState(this.tid);
}

class _tAttendance_HomeState extends State<tAttendance_Home> {
 var tid;
 String barcode;
 _tAttendance_HomeState(this.tid);
          ...
          final snapShot = await Firestore.instance
                  ...
                  if (snapShot == null || !snapShot.exists) {
                    // Document with id == docId doesn't exist.
                       MessageBox('Error', 'Teacher Id Does Not Exist', 'Try Again'); 
                  }else{
                        DateTime now = new DateTime.now();
                        DateTime date = new DateTime(now.year, now.month, now.day);
                          //print(DateTime.now());
                          ...
                          crudObj.teacherattendanceData({
                            ...
                            'Attendance' : date,
                            'Tid' : barcode.toUpperCase()
                            //'Uid' : _uid
                          }).then((result){
                            //Navigator.of(context).pushReplacementNamed('/HomePage');
                             MessageBox('Success', 'Teacher Marked As Present', 'OK'); 
